...
<link rel="stylesheet" type="text/css" href="skin/css/base.css">
    ...
    <script type="application/javascript">
       <%--
       javascript:必须为每一个对象设置属性
       jquery：设置一组的元素，相当于为这一组中的每一个元素都设置一遍属性
       --%>
        function chooseAll() {
           $("#table_list input").attr("checked","checked");
        }
        ...
        function batchDelete() {
            var  ids=[];
            $("#table_list").find("input:checked").each(function (index,item) {
                   var cid=$(this).val();
                   ids.push(cid);
            });
                ...
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/cust/del/"+ids,
                    date:{"_method":"delete"},
...
<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
	<td><input name="id" type="checkbox" id="${customer.id}" value="${customer.id}" class="np"></td>
	<td>${i.count}</td>
	<td>${customer.companyperson}</td>
	<td align="center">${comname}</td>
	<td>
